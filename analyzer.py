...
class AnalysisResult:
    ...
    long_term_outlook: str = ""  # 中长期展望（1-3个月，适合场外ETF投资者参考）
...
### 6. 中长期趋势分析（场外ETF投资者必看）
**注意：以下分析适合通过支付宝、天天基金等平台购买场外ETF基金的投资者参考**

- **中期均线判断**：MA20 > MA60 表示中期多头
- **长期均线判断**：MA60 > MA120 表示长期多头
- **中长期买点**：
  - 价格回踩 MA60 获支撑 = 中期好买点
  - 价格回踩 MA120 获支撑 = 长期好买点
- **场外ETF特点**：T+1交易、申购赎回有延迟，不适合短线操作
- **操作建议要点**：
  - 关注趋势方向而非精确点位
  - 分批建仓优于一次性买入
  - 中长期持有，减少操作频率
...
```json
{
    ...
    "dashboard": {
        ...
        "etf_investor_guide": {
            "long_term_trend": "中长期趋势判断（基于MA60/MA120）",
            "trend_health": "趋势健康度：强势上涨/温和上涨/震荡整理/调整下跌",
            "buy_timing": "场外ETF申购时机建议",
            "position_suggestion": "建议仓位比例（如：可配置3成仓位）",
            "hold_period": "建议持有周期（如：中期持有1-3个月）",
            "key_attention": "重点关注事项（如：关注季报业绩、政策变化等）"
        }
    },
    ...
    "long_term_outlook": "中长期1-3个月展望（场外ETF投资者重点参考）",
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = self._openai_client.chat.completions.create(
                    ...
                    max_tokens=generation_config.get('max_output_tokens', 64000),
                )
