...
class StockAnalysisPipeline:
    ...
    def process_single_stock(
        ...
        try:
            # Step 1: 获取并保存数据（skip_fetch 模式下跳过）
            if skip_fetch:
                logger.info(f"[{code}] 跳过数据获取，使用已有数据")
                success = True
                error = None
            else:
    ...
    def run(
        ...
        Args:
            ...
            dry_run: 是否仅获取数据不分析
            analyze_only: 是否仅分析不获取数据（使用已有数据）
        ...
        mode = '仅获取数据' if dry_run else ('仅分析' if analyze_only else '完整分析')
        logger.info(f"并发数: {self.max_workers}, 模式: {mode}")
