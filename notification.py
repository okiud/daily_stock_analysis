...
class NotificationService:
    ...
    def generate_daily_report(
        ...
        for result in sorted_results:
            ...
            if hasattr(result, 'long_term_outlook') and result.long_term_outlook:
                outlook_lines.append(f"- **🏦 中长期（1-3月，场外ETF参考）**：{result.long_term_outlook}")
            if outlook_lines:
                report_lines.extend([
                    "#### 🔮 市场展望",
                    *outlook_lines,
    ...
    def generate_dashboard_report(
        ...
        for result in sorted_results:
            ...
            # ========== 场外ETF投资者指南（新增）==========
            etf_guide = dashboard.get('etf_investor_guide', {}) if dashboard else {}
            if etf_guide:
                report_lines.extend([
                    "### 🏦 场外ETF投资者参考",
                    "",
                ...
                if etf_guide.get('long_term_trend'):
                    report_lines.append(f"**📈 中长期趋势**: {etf_guide['long_term_trend']}")
                if etf_guide.get('trend_health'):
                    report_lines.append(f"**💪 趋势健康度**: {etf_guide['trend_health']}")
                if etf_guide.get('buy_timing'):
                    report_lines.append(f"**⏰ 申购时机**: {etf_guide['buy_timing']}")
                if etf_guide.get('position_suggestion'):
                    report_lines.append(f"**💰 建议仓位**: {etf_guide['position_suggestion']}")
                if etf_guide.get('hold_period'):
                    report_lines.append(f"**📅 持有周期**: {etf_guide['hold_period']}")
                if etf_guide.get('key_attention'):
                    report_lines.append(f"**👀 重点关注**: {etf_guide['key_attention']}")
                
                report_lines.append("")
            
            # 中长期展望（单独字段）
            if hasattr(result, 'long_term_outlook') and result.long_term_outlook:
                if not etf_guide:  # 避免重复显示
                    report_lines.extend([
                        "### 🏦 中长期展望（场外ETF参考）",
                        "",
                        f"{result.long_term_outlook}",
                        "",
                    ])
